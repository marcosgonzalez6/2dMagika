...
public class Player extends CreatureBase {
    ...
    private final int defaultPlayerHealth = 75;
    ...
    public void tick() {
        ...
        //Regenerate Health
        if(handler.getKeyManager().regenerateHealthBut) {
        	this.health = 75;
        }
    ...
    public void checkAttacks(){
        ...
        for(EntityBase e : handler.getWorld().getEntityManager().getEntities()){
            if(e.equals(this))
                continue;
            if(e.getCollisionBounds(0, 0).intersects(ar) && !(e instanceof Chest)){
                e.hurt(attack);
                System.out.println(e + " has " + e.getHealth() + " lives.");
                return;
            }
            else if(e.getCollisionBounds(0, 0).intersects(ar) && (e instanceof Chest)){
                e.hurt(attack);
//                Chest et = (Chest) e;
//                et.deposit(this.getInventory().getInventoryItems());
                return;
            }
        }
